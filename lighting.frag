     ...
     for (int i = 0; i < NumLights; i++) {
          ...
          if (LightOn[i] != 0) {
               ...
               if (Lights[i].type == 2) {
                    ...
                    float attenuation = 1.0 / (1.0 + 0.09 * distance + 0.032 * distance * distance);  // Added distance-based attenuation to smooth the lighting.
                    rgb += diff*vec3(Lights[i].diffuse*Materials[Material].diffuse) * attenuation;
                    if (diff > 0.0) {
                         // Specular term
                         float spec = pow(max(0.0f, dot(Normal, HalfVector)), Materials[Material].shininess);
                         rgb += spec*vec3(Lights[i].specular*Materials[Material].specular);
